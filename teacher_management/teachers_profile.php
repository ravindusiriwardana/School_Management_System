...
// Fix include paths: go up one directory since this file is inside teacher_management/
include_once '../config/dbConnection.php';
include_once '../navbar.php';

// Redirect if user not logged in or not a teacher
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'teacher') {
    header("Location: ../login.php");
    exit();
}

// Get teacher details from DB
$id = $_SESSION['user_id'];
if (!isset($conn)) {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #ffffff; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 20px; }
        .profile-container { background: white; border-radius: 20px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.1); border: 1px solid #e5e7eb; overflow: hidden; max-width: 500px; width: 100%; position: relative; animation: fadeInUp 0.6s ease-out; }
        .profile-header { background: #2563eb; padding: 40px 30px 30px; text-align: center; position: relative; }
        .profile-header::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="grid" width="10" height="10" patternUnits="userSpaceOnUse"><path d="M 10 0 L 0 0 0 10" fill="none" stroke="rgba(255,255,255,0.1)" stroke-width="1"/></pattern></defs><rect width="100" height="100" fill="url(%23grid)"/></svg>'); opacity: 0.3; }
        .profile-avatar { width: 100px; height: 100px; background: rgba(255, 255, 255, 0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; position: relative; z-index: 1; border: 4px solid rgba(255, 255, 255, 0.3); overflow: hidden; cursor: pointer; transition: all 0.3s ease; }
        ...
        .avatar-upload { position: absolute; bottom: 0; right: 0; width: 32px; height: 32px; background: #16a34a; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; border: 3px solid white; transition: all 0.3s ease; }
        ...
        .info-value { background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 12px; padding: 16px 18px; font-size: 16px; color: #1e293b; font-weight: 500; transition: all 0.3s ease; position: relative; overflow: hidden; }
        ...
        .btn { display: flex; align-items: center; justify-content: center; padding: 14px 20px; border-radius: 12px; text-decoration: none; font-weight: 600; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; transition: all 0.3s ease; position: relative; overflow: hidden; }
        ...
        .btn-info:hover { background: #1e40af; transform: translateY(-2px); }
...
<body>
    ...
    <div class="info-item full-width">
        ...
        <div class="info-value"><?php echo htmlspecialchars($user['bio'] ?? '-'); ?></div>
            ...
            <div class="action-buttons">
                <a href="../teacher_management/edit_teacher.php" class="btn btn-info"><i class="fas fa-user-edit"></i> Edit Profile</a>
                <a href="../subject_management/subject_list.php" class="btn btn-primary"><i class="fas fa-list-alt"></i> Manage Subjects</a>
                <a href="../student_management/student_list.php" class="btn btn-primary"><i class="fas fa-user-plus"></i> Student List</a>
    ...
    <script>
        ...
        function showMessage(message, type) {
            ...
            messageDiv.style.cssText = `position: fixed; top: 20px; right: 20px; padding: 15px 20px; border-radius: 8px; color: white; font-weight: 600; z-index: 1000; animation: slideIn 0.3s ease-out; background: ${type === 'success' ? '#16a34a' : '#dc2626'};`;
            messageDiv.textContent = message;
            document.body.appendChild(messageDiv);
            setTimeout(() => { messageDiv.style.animation = 'slideOut 0.3s ease-in'; setTimeout(() => messageDiv.remove(), 300); }, 3000);
        }
